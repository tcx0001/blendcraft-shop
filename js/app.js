...
function footerHTML(){
  return `<div class="container">
    <div class="footer-grid">
      ...
      <div><h4>Contact</h4><a href="mailto:support@blendcraft.example">support@blendcraft.example</a><a href="contact.html">We reply within 24-48h</a><a href="shipping.html">Worldwide shipping</a></div>
    </div>
...
const STORE_EMAIL = 'orders@blendcraft.example';
...
function renderCartPage(){
  ...
  const rows = c.map(i=>{ const p=byId(i.id); if(!p) return '';
    return `<div class="cart-item">
      <div class="thumb">${artFor(p.art)}</div>
...
function productCard(p){
  return `<article class="product-card">
    <div class="product-media">${p.badge?`<span class="product-badge">${p.badge}</span>`:''}${artFor(p.art)}</div>
...
function renderProductDetail(){
  ...
  wrap.innerHTML = `<div class="pd">
    <div class="pd-media">${artFor(p.art)}</div>
    <div class="pd-info">
...
function initContact(){
  ...
  f.addEventListener('submit', e=>{ e.preventDefault(); toast('Message sent ✓'); f.reset(); });
}
